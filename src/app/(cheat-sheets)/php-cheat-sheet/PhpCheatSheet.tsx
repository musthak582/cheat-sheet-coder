...
import { phpCodeBlocks } from '../../../../public/data/php-code'
import { FaBars, FaTimes } from 'react-icons/fa';
const PhpCheatSheet = () => {
  // State to track the currently selected section
  const [selectedSection, setSelectedSection] = useState(phpCodeBlocks[0].id);
  // for toggle sidebar
  ...
  return (
    <div className="w-full flex flex-col bg-white xl:px-[100px] sm:px-4 px-2">
      <div className={`w-full bg-zinc-100 flex lg:flex-col lg:items-center lg:justify-center lg:gap-4 gap-2 sm:py-4 py-2`}>
        ...
        <h2 className='lg:text-3xl md:text-2xl text-[20px] text-center font-extrabold text-slate-900 '>PHP Cheat Sheet</h2>
        <p className='max-w-4xl hidden lg:inline text-[20px] text-slate-700 text-center'>Access our free PHP Cheat Sheet to master syntax, functions, and database integration. This comprehensive guide includes practical examples for efficient PHP programming. Perfect for quick reference!</p>
      </div>
          ...
          {phpCodeBlocks.map((section) => (
            selectedSection === section.id && (
              <div key={section.id} className='h-full bg-[#1E1E1E] '>
                <pre>
                  <code className="language-php text-[18px]">
                    {section.content}
                  </code>
                </pre>
              </div>
