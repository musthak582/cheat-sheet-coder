...
import { pythonCodeBlocks } from '../../../../public/data/python-code'
import { FaBars, FaTimes } from 'react-icons/fa';
const JavaScriptCheatSheet = () => {
  // State to track the currently selected section
  const [selectedSection, setSelectedSection] = useState(pythonCodeBlocks[0].id);
  // for toggle sidebar
  ...
  return (
    <div className="w-full flex flex-col bg-white xl:px-[100px] sm:px-4 px-2">
      <div className={`w-full bg-zinc-100 flex lg:flex-col lg:items-center lg:justify-center lg:gap-4 gap-2 sm:py-4 py-2`}>
        ...
        <h2 className='lg:text-3xl md:text-2xl text-[20px] text-center font-extrabold text-slate-900 '>Python Cheat Sheet</h2>
        <p className='max-w-4xl hidden lg:inline text-[20px] text-slate-700 text-center'>Discover Python essentials with our free Python Cheat Sheet. Perfect for beginners and developers. Get quick references for syntax, data structures, and common libraries to write efficient and powerful Python code effortlessly.</p>
      </div>
          ...
          {pythonCodeBlocks.map((section) => (
            selectedSection === section.id && (
              <div key={section.id} className='h-full bg-[#1E1E1E] '>
                <pre>
                  <code className="language-python text-[18px]">
                    {section.content}
                  </code>
                </pre>
              </div>
