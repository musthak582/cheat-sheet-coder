...
import { csharpCodeBlocks } from '../../../../public/data/csharp-code'
import { FaBars, FaTimes } from 'react-icons/fa';
const PhpCheatSheet = () => {
  // State to track the currently selected section
  const [selectedSection, setSelectedSection] = useState(csharpCodeBlocks[0].id);
  // for toggle sidebar
  ...
  return (
    <div className="w-full flex flex-col bg-white xl:px-[100px] sm:px-4 px-2">
      <div className={`w-full bg-zinc-100 flex lg:flex-col lg:items-center lg:justify-center lg:gap-4 gap-2 sm:py-4 py-2`}>
        ...
        <p className='max-w-4xl hidden lg:inline text-[20px] text-slate-700 text-center'>Access our free C# Cheat Sheet to master syntax, classes, and LINQ. This comprehensive guide includes practical examples for efficient C# programming. Perfect for quick reference!</p>
      </div>
          ...
          {csharpCodeBlocks.map((section) => (
            selectedSection === section.id && (
              <div key={section.id} className='h-full bg-[#1E1E1E] '>
                <pre>
                  <code className="language-csharp text-[18px]">
                    {section.content}
                  </code>
                </pre>
              </div>
